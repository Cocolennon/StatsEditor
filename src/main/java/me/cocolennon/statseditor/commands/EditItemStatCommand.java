...
public class EditItemStatCommand implements TabExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if(args.length == 0) {
            sender.sendMessage("§3[§dStatsEditor§3]§f: §cUsage: /"+label+" <player> <stat name> <mob name> <value>");
            return false;
        }
        ...
        if(mobStatistics.contains(statistic)) {
            player.sendMessage("§3[§dStatsEditor§3]§f: §cThe statistic you inputted need to be used with /edit-mob-stat.");
            return false;
        }
