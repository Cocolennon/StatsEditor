...
public class EditMobStatCommand implements TabExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if(args.length == 0) {
            sender.sendMessage("§3[§dStatsEditor§3]§f: §cUsage: /"+label+" <player> <stat name> <item name> <value>");
            return false;
        }

        Player player = Bukkit.getServer().getPlayer(args[0]);
        if(player == null) {
            sender.sendMessage("§3[§dStatsEditor§3]§f: §cPlayer is either not online or does not exist.");
            return false;
        }

        if(args.length < 2) {
            sender.sendMessage("§3[§dStatsEditor§3]§f: §cYou must input a statistic to edit.");
            return false;
        }

        if(args.length < 3) {
            sender.sendMessage("§3[§dStatsEditor§3]§f: §cYou must input an item to set the statistic of.");
            return false;
        }

        if(args.length < 4) {
            sender.sendMessage("§3[§dStatsEditor§3]§f: §cYou must input a value to set the statistic at.");
            return false;
        }
        ...
        if(itemStatistics.contains(statistic)) {
            player.sendMessage("§3[§dStatsEditor§3]§f: §cThe statistic you inputted need to be used with /edit-item-stat.");
            return false;
        }
        ...
        if(!mobStatistics.contains(statistic)) {
            player.sendMessage("§3[§dStatsEditor§3]§f: §cThe statistic you inputted need to be used with /edit-stat.");
            return false;
        }
        ...
        try {
            ...
            player.sendMessage("§3[§dStatsEditor§3]§f: §cEither statistic or the item you inputted does not exist.");
            return false;
        }

        sender.sendMessage("§3[§dStatsEditor§3]§f: §aStatistic has successfully been edited.");
